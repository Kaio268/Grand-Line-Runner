...
-- Visual language borrowed from the FeedYourCrewmates tutorial card:
-- dark navy parchment fills, gold gradient borders, a cream SpecialElite
-- title, SourceSans body copy and Cartoon-font buttons.
local THEME = {
...
-- Gold border with a vertical highlight->shadow gradient, matching the
-- UIStroke + UIGradient pairing used across the FeedYourCrewmates frames.
...
local function NamiShopScreen(props)
	...
	return e("Frame", {
		...
		-- Nami art kept as a faint texture so the navy card reads first.
