...
-- Shared design language with the SELL / QUEST / GIFTS menus: semi-transparent
-- black card, brighter gold gradient borders + edge glow, a gold SpecialElite pill
-- badge title and a red gradient close button.
local SHELL = {
...
-- gold SpecialElite pill badge that straddles the very top edge (like QUEST/GIFTS)
...
-- red gradient close button that straddles the top-right corner (like QUEST/GIFTS)
...
-- dropdown filter (Normal / Golden / Diamond) shown in the header next to the tabs
local function categoryDropdown(props)
	...
	for _, c in ipairs(categories) do
		if tostring(c.id) == tostring(active) then
			activeLabel = tostring(c.label or "Filter")
		end
	end
	...
	for index, c in ipairs(categories) do
		local isActive = tostring(c.id) == tostring(active)
		optionChildren["Opt" .. tostring(c.id)] = e("TextButton", {
			AutoButtonColor = false,
			...
			Outline = e("UIStroke", {
				...
				Grad = e("UIGradient", { Rotation = 90, Color = ColorSequence.new(Color3.fromRGB(80, 80, 80), Color3.fromRGB(0, 0, 0)) }),
			} or nil),
			Sheen = e("UIGradient", {
				Rotation = 90,
				Color = isActive and ColorSequence.new(Color3.fromRGB(255, 250, 222), Color3.fromRGB(216, 168, 64))
					or ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(200, 200, 200)),
			}),
			Title = e("TextLabel", {
				BackgroundTransparency = 1,
	...
	return e("Frame", {
		...
		Button = e("TextButton", {
			...
			Outline = e("UIStroke", { ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Color = Color3.fromRGB(0, 0, 0), Thickness = 1.8 }),
			Sheen = e("UIGradient", {
				Rotation = 90,
		...
		Menu = open and e("Frame", {
			...
			Corner = e("UICorner", { CornerRadius = UDim.new(0, 10) }),
			Stroke = e("UIStroke", { ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Color = SHELL.GoldHighlight, Thickness = 1.35 }),
			Pad = e("UIPadding", {
