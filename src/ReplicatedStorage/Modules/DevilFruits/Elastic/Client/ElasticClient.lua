...
local RubberLaunchMath = require(
	DevilFruits:WaitForChild("Elastic"):WaitForChild("Shared"):WaitForChild("RubberLaunchMath")
)

local ElasticClient = {}
ElasticClient.__index = ElasticClient

local FRUIT_NAME = "Elastic Fruit"
...
local function getRubberLaunchFxAsset()
	...
	local gomuFolder = vfxFolder and (vfxFolder:FindFirstChild("Elastic") or vfxFolder:FindFirstChild("Gomu"))
...
function ElasticClient.Create(config)
	config = config or {}
	local self = setmetatable({}, ElasticClient)
...
function ElasticClient:BeginPredictedRequest(abilityName, fallbackBuilder)
...
function ElasticClient:BuildRequestPayload(abilityName, _abilityEntry, fallbackBuilder)
	if abilityName == ABILITY_NAME then
...
function ElasticClient:HandleEffect(targetPlayer, abilityName, payload)
	if abilityName ~= ABILITY_NAME then
		return false
	end
...
function ElasticClient:HandleStateEvent()
	return false
end

function ElasticClient:HandleEquipped()
	return false
end

function ElasticClient:HandleUnequipped()
	clearGomuHighlight(self)
...
function ElasticClient:HandleCharacterRemoving()
	clearGomuHighlight(self)
	clearLaunchVfxForPlayer(self, self.player)
	restoreRubberLaunchArmsForPlayer(self, self.player)
end

function ElasticClient:HandlePlayerRemoving(leavingPlayer)
