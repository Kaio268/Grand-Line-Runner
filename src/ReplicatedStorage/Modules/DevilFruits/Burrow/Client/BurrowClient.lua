...
local MoguAnimationController = require(
	Modules:WaitForChild("DevilFruits"):WaitForChild("Burrow"):WaitForChild("Client"):WaitForChild("MoguAnimationController")
)
local MoguVfxController = require(
	Modules:WaitForChild("DevilFruits"):WaitForChild("Burrow"):WaitForChild("Client"):WaitForChild("MoguVfxController")
)
local MoguBurrowShared = require(
	Modules:WaitForChild("DevilFruits"):WaitForChild("Burrow"):WaitForChild("Shared"):WaitForChild("MoguBurrowShared")
)
local GameSounds = require(Modules:WaitForChild("GameSounds"))

local BurrowClient = {}
BurrowClient.__index = BurrowClient

local FRUIT_NAME = "Burrow Fruit"
...
local function logWarn(message, ...)
	if not DiagnosticLogLimiter.ShouldEmit("BurrowClient:WARN", DiagnosticLogLimiter.BuildKey(message, ...), WARN_COOLDOWN) then
		return
	end
...
local function logInfo(message, ...)
	...
	if not DiagnosticLogLimiter.ShouldEmit("BurrowClient:INFO", DiagnosticLogLimiter.BuildKey(message, ...), INFO_COOLDOWN) then
		return
	end
...
function BurrowClient.Create(config)
	config = config or {}

	local self = setmetatable({}, BurrowClient)
...
function BurrowClient:SetDiagnosticAttribute(name, value)
...
function BurrowClient:RecordPivotWrite(didWrite)
	if didWrite and self.diagnostics then
		self.diagnostics.PivotWrites += 1
	end
end

function BurrowClient:RecordCleanup()
	if self.diagnostics then
		self.diagnostics.CleanupCount += 1
	end
end

function BurrowClient:SetBurrowStatePhase(targetPlayer, burrowState, phase, reason)
...
function BurrowClient:PublishDiagnostics(force)
...
function BurrowClient:SetBurrowInputKeyState(keyCode, isPressed)
...
function BurrowClient:GetBurrowInputAxes()
	local forwardAxis = 0
	local rightAxis = 0
...
function BurrowClient:GetBurrowActivationDirection(rootPart)
...
function BurrowClient:GetBurrowSurfaceDirection(rootPart, fallbackDirection)
	local inputDirection = self:GetCameraRelativeBurrowDirection(rootPart)
	if inputDirection.Magnitude > MIN_DIRECTION_MAGNITUDE then
		return inputDirection.Unit
	end
...
function BurrowClient:ClearVisualBurrowOffset(targetPlayer, shouldTween, duration, onComplete)
...
function BurrowClient:HasVisualBurrowOffset(targetPlayer)
	local visualState = self.visualBurrowStates[targetPlayer]
	if not visualState then
		return false
	end
...
function BurrowClient:ApplyVisualBurrowOffset(targetPlayer, abilityConfig, options)
...
function BurrowClient:ApplyStartupVisualSink(targetPlayer, abilityConfig)
...
function BurrowClient:ScheduleStartupVisualSink(targetPlayer, burrowState, abilityConfig)
...
function BurrowClient:SnapVisualBurrowOffset(targetPlayer)
	local visualState = self.visualBurrowStates[targetPlayer]
	if not visualState then
		return false
	end
...
function BurrowClient:ApplyConceal(targetPlayer, transparency)
...
function BurrowClient:ApplyConcealWhenReady(targetPlayer, burrowState)
...
function BurrowClient:CancelCameraRestoreTween(humanoid)
...
function BurrowClient:ApplyBurrowCameraStabilization(ownerState, humanoid, surfacePosition, buriedPosition, abilityConfig)
...
function BurrowClient:RestoreBurrowCameraStabilization(ownerState, reason)
...
function BurrowClient:PlayBurrowEntryVfx(targetPlayer, burrowState, startPosition, abilityConfig, reason)
...
function BurrowClient:CompleteBurrowRootTransition(targetPlayer, burrowState, startPosition, abilityConfig, reason)
...
function BurrowClient:ScheduleBurrowRootTransition(targetPlayer, burrowState, startPosition, abilityConfig)
	if not targetPlayer or self.burrowStates[targetPlayer] ~= burrowState then
		return false
	end
...
function BurrowClient:TriggerBurrowEntryCue(targetPlayer, burrowState, startPosition, abilityConfig)
	if not targetPlayer or self.burrowStates[targetPlayer] ~= burrowState then
		return false
	end
...
function BurrowClient:TriggerBurrowMovementCue(targetPlayer, burrowState, startPosition, abilityConfig)
	if not targetPlayer or self.burrowStates[targetPlayer] ~= burrowState then
		return false
	end
...
function BurrowClient:ApplyBurrowPhysicsState(ownerState, character, humanoid)
...
function BurrowClient:RestoreBurrowPhysicsState(ownerState, reason)
	if type(ownerState) ~= "table" then
		return false
	end
...
function BurrowClient:StabilizeBurrowStartup(character, rootPart, direction, abilityConfig)
	if not character or not rootPart then
		return nil
	end
...
function BurrowClient:RestoreStartupStabilization(pending, reason)
...
function BurrowClient:ApplyStartupMovementLock(ownerState, character, rootPart, humanoid, lockPosition, direction, reason)
...
function BurrowClient:ReleaseStartupMovementLock(ownerState, reason)
...
function BurrowClient:ResolveGroundedActivationSurface(character, rootPart, abilityConfig)
	if not character or not rootPart then
...
function BurrowClient:ShowGroundedRequiredFeedback(reason)
	local now = os.clock()
...
function BurrowClient:ClearLegacyAirborneQueueDiagnostics()
...
function BurrowClient:RejectGroundedOnlyBurrow(reason, dropDistance, shouldShowFeedback)
...
function BurrowClient:ValidateGroundedOnlyBurrowStart(shouldShowFeedback)
...
function BurrowClient:BeginLocalStartFeedback(direction)
...
function BurrowClient:CancelLocalStartFeedback(reason)
...
function BurrowClient:CancelServerConfirmedBurrowStart(reason)
...
function BurrowClient:UpdatePendingLocalStartFeedback()
	local pending = self.pendingBurrowFeedback
	if not pending then
		return false
	end
...
function BurrowClient:ScheduleBurrowEntryCue(targetPlayer, burrowState, startPosition, abilityConfig)
...
function BurrowClient:ScheduleBurrowMovementCue(targetPlayer, burrowState, startPosition, abilityConfig)
	local stageConfig = getAnimationStageConfig("Start", abilityConfig)
...
function BurrowClient:ScheduleBurrowStartupWatchdog(targetPlayer, burrowState, abilityConfig)
...
function BurrowClient:ClearConceal(targetPlayer)
...
function BurrowClient:RevealConcealedCharacter(targetPlayer)
	local concealState = self.concealStates[targetPlayer]
...
function BurrowClient:RepairActiveConceal(targetPlayer, burrowState, now)
	if
		not targetPlayer
...
function BurrowClient:GetLocalBurrowState()
	return self.burrowStates[self.player]
end

function BurrowClient:AttachBurrowLifecycleCleanup(targetPlayer, burrowState)
...
function BurrowClient:RequestSurface(reason)
...
function BurrowClient:StartBurrow(targetPlayer, payload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function BurrowClient:FinishResolve(targetPlayer, burrowState, reason)
	if not targetPlayer or type(burrowState) ~= "table" then
		return false
	end
...
function BurrowClient:StopBurrow(targetPlayer, payload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function BurrowClient:HandleInputBegan(input, gameProcessed)
...
function BurrowClient:HandleInputEnded(input)
	return self:SetBurrowInputKeyState(input and input.KeyCode, false)
end

function BurrowClient:BeginPredictedRequest(abilityName, fallbackBuilder)
...
function BurrowClient:BuildRequestPayload(abilityName, _abilityEntry, fallbackBuilder)
	if abilityName == ABILITY_NAME then
...
function BurrowClient:UpdateLocalBurrowState(burrowState, dt, now, abilityConfig)
	local character = getCharacter(self.player)
	local rootPart = self.getLocalRootPart()
	local humanoid = self.getHumanoid()
	if not character or not rootPart or not humanoid or humanoid.Health <= 0 then
...
function BurrowClient:UpdateTrailState(targetPlayer, burrowState, now, abilityConfig)
	if
		not burrowState
...
function BurrowClient:HandleEffect(targetPlayer, abilityName, payload)
	if abilityName ~= ABILITY_NAME then
		return false
	end
...
function BurrowClient:HandleStateEvent(eventName, abilityName, value)
	if abilityName ~= ABILITY_NAME then
		return false
	end
...
function BurrowClient:Update(dt)
...
function BurrowClient:HandleEquipped()
	return false
end

function BurrowClient:HandleUnequipped()
	self:HandleCharacterRemoving()
	return false
end

function BurrowClient:HandleCharacterRemoving()
