...
local DEFAULT_ANIMATION_KEY_BY_STAGE = {
	[STAGE_START] = "Burrow.MoleDigStart",
	[STAGE_RESOLVE] = "Burrow.MoleDigEnd",
}
...
local function logInfo(message, ...)
	...
	if not DiagnosticLogLimiter.ShouldEmit("BurrowClientAnimationController:INFO", DiagnosticLogLimiter.BuildKey(message, ...), INFO_COOLDOWN) then
		return
	end
...
local function logWarn(message, ...)
	if not DiagnosticLogLimiter.ShouldEmit("BurrowClientAnimationController:WARN", DiagnosticLogLimiter.BuildKey(message, ...), WARN_COOLDOWN) then
		return
	end
...
local function resolveAnimationAsset(stageKey, stageConfig)
	...
	local animation, descriptor = AnimationResolver.GetAnimation(animationKey, {
		Context = string.format("Burrow.Client.%s", tostring(stageKey)),
	})
	if animation then
...
local function playAnimationForPlayer(targetPlayer, stageKey, abilityConfig)
	...
	AnimationLoadDiagnostics.LogTrackPlay(
		track,
		SOURCE_LABEL,
		string.format("Burrow.Client.%s", tostring(stageKey)),
