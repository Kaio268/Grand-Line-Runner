...
local PhantomClient = {}
PhantomClient.__index = PhantomClient

local FRUIT_NAME = "Phantom Fruit"
...
local DEFAULT_ANIMATION_KEY = "Phantom.Suke1"
...
local SOURCE_LABEL = "ReplicatedStorage.Modules.DevilFruits.Suke.Client.PhantomClient"
...
local function logWarn(message, ...)
	if not DiagnosticLogLimiter.ShouldEmit("PhantomClient:WARN", DiagnosticLogLimiter.BuildKey(message, ...), WARN_COOLDOWN) then
		return
	end
...
local function getAuthoredVfxTemplate(effectName)
	...
	local sukeFolder = vfxFolder and (vfxFolder:FindFirstChild("Phantom") or vfxFolder:FindFirstChild("Suke"))
...
function PhantomClient.Create(config)
	config = config or {}

	local self = setmetatable({}, PhantomClient)
...
function PhantomClient:ClearFadeWindup(targetPlayer, immediate)
...
function PhantomClient:ActivateFadeFromWindup(targetPlayer, windup)
...
function PhantomClient:ArmFadeActivation(targetPlayer, windup, animationConfig)
	local animationState = windup.AnimationState
...
function PhantomClient:StartFadeWindup(targetPlayer, payload, isPredicted)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function PhantomClient:BeginPredictedRequest(abilityName, fallbackBuilder)
...
function PhantomClient:BuildRequestPayload(abilityName, _abilityEntry, fallbackBuilder)
	if abilityName == ABILITY_NAME then
		return nil
	end
...
function PhantomClient:ClearFade(targetPlayer, immediate)
...
function PhantomClient:StartFade(targetPlayer, payload, serverConfirmed)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function PhantomClient:HandleEffect(targetPlayer, abilityName, payload)
	if abilityName ~= ABILITY_NAME then
		return false
	end
...
function PhantomClient:HandleStateEvent(eventName, abilityName)
...
function PhantomClient:HandleEquipped()
	return false
end

function PhantomClient:HandleUnequipped()
	self:HandleCharacterRemoving()
	return false
end

function PhantomClient:HandleCharacterRemoving()
	self:ClearFadeWindup(self.player, true)
	self:ClearFade(self.player, true)
end

function PhantomClient:HandlePlayerRemoving(leavingPlayer)
