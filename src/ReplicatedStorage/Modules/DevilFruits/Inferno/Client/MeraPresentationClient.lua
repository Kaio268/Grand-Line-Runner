...
local MeraFolder = DevilFruits:WaitForChild("Inferno")
...
local function getMeraAudioConfig(abilityName)
	local abilityConfig = DevilFruitConfig.GetAbility("Inferno Fruit", abilityName)
...
function MeraPresentationClient:GetAnimationConfig(moveName)
	local abilityConfig = DevilFruitConfig.GetAbility("Inferno Fruit", moveName)
...
function MeraPresentationClient:EnsureFireBurstAnimation(state)
	...
	local track, animationPath, loadFailure, selectedCandidate = self:PlayAnimation(targetPlayer, "FireBurst", "Flame burst", "Inferno.FlameBurstR6")
...
function MeraPresentationClient:PlayFlameDashStartup(targetPlayer, _payload, _isPredicted)
	...
	local track, animationPath, loadFailure, selectedCandidate = self:PlayAnimation(targetPlayer, "FlameDash", "Flame Dash", "Inferno.FlameDash", {
		DeferPlay = true,
	})
		...
		if ok then
			AnimationLoadDiagnostics.LogTrackPlay(
				...
				string.format(
					...
					tostring(selectedCandidate and selectedCandidate.LogicalKey or "Inferno.FlameDash"),
					tostring(animationPath),
					tostring(selectedCandidate and selectedCandidate.Source),
					fadeTime,
					playbackSpeed,
