...
local PhoenixShared = require(DevilFruits:WaitForChild("Phoenix"):WaitForChild("Shared"):WaitForChild("PhoenixShared"))

local PhoenixClient = {}
PhoenixClient.__index = PhoenixClient

local DEFAULT_PHOENIX_FRUIT_NAME = PhoenixShared.FruitName
...
local PHOENIX_REBIRTH = PhoenixShared.Passives.PhoenixRebirth
local DEBUG_FLIGHT = false
local MIN_DIRECTION_MAGNITUDE = 0.01
local MIN_FLIGHT_MOVE_INPUT_MAGNITUDE = 0.05
...
local DEFAULT_PHOENIX_END_DESCEND_SPEED = 72
...
local PHOENIX_FLIGHT_ANIMATION_KEYS = {
	Loop = "Phoenix.PhoenixFlightLoop",
	Idle = "Phoenix.PhoenixFlightIdle",
}
...
local function phoenixCooldownLog(message, ...)
	print("[PhoenixCooldown] " .. string.format(message, ...))
end
	...
	task.spawn(function()
		...
		for _, audioKey in pairs(PHOENIX_SHIELD_AUDIO_KEYS) do
			...
			if soundId and not seenSoundIds[soundId] then
				...
				sound.Name = "PhoenixFlameShieldPreload"
...
local function preloadPhoenixRebirthAudio(self)
	...
	task.spawn(function()
		...
		local sound = Instance.new("Sound")
		sound.Name = "PhoenixRebirthPreload"
		sound.SoundId = soundId
		pcall(function()
...
function PhoenixClient.new(config)
	local self = setmetatable({}, PhoenixClient)
...
function PhoenixClient.Create(config)
	return PhoenixClient.new(config)
end

function PhoenixClient:EnsureProtectionRegistration()
...
function PhoenixClient:EnsurePhoenixRebirthPlaceholderHook()
...
function PhoenixClient:GetActiveFlightInterruptHitEffect()
	local player = self.player
	if not player then
		return nil
	end
...
function PhoenixClient:StopPhoenixFlightForHitEffect(effectName, source)
...
function PhoenixClient:StopPhoenixFlightForActiveHitEffect(source)
...
function PhoenixClient:EnsureFlightHitEffectHook()
...
function PhoenixClient:IsPhoenixRebirthProtected()
	local player = self.player
	if not player then
		return false
	end
...
function PhoenixClient:HandlePhoenixRebirthTriggered(triggeredAt)
...
function PhoenixClient:PlayPhoenixRebirthVisual(targetPlayer, payload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function PhoenixClient:ResolveEquippedFruitName()
	local player = self.player
	if not player then
...
function PhoenixClient:GetPhoenixFlightKeyCode()
	local abilityConfig = DevilFruitConfig.GetAbility(self.phoenixFruitName, self.phoenixFlightAbility)
...
function PhoenixClient:ShouldLogFlightDebug(timerKey, now, interval)
	if not DEBUG_FLIGHT then
		return false
	end
...
function PhoenixClient:IsPhoenixFlightActive(now)
	now = now or os.clock()
...
function PhoenixClient:CreatePhoenixFlightSound(targetPlayer, audioKey, looped)
	...
	local sound = Instance.new("Sound")
	sound.Name = "PhoenixFlight" .. tostring(audioKey)
	sound.SoundId = soundId
...
function PhoenixClient:PlayPhoenixFlightOneShot(targetPlayer, audioKey)
...
function PhoenixClient:PlayPhoenixFlightCueOnce(targetPlayer, cueName, audioKey)
	local state = getOrCreatePhoenixFlightAudioState(self, targetPlayer)
	if not state then
		return nil
	end
...
function PhoenixClient:StartPhoenixFlightLoopSound(targetPlayer)
	local state = getOrCreatePhoenixFlightAudioState(self, targetPlayer)
	if not state then
		return nil
	end
...
function PhoenixClient:StopPhoenixFlightLoopSound(targetPlayer)
	local audioStates = self.phoenixFlightAudioStates
	local state = audioStates and audioStates[targetPlayer] or nil
	stopPhoenixFlightLoopSoundState(state)
end

function PhoenixClient:StopPhoenixFlightAudioForPlayer(targetPlayer, options)
...
function PhoenixClient:CreatePhoenixShieldSound(targetPlayer, audioKey, looped)
	local rootPart = getPlayerRootPart(targetPlayer)
	if not rootPart then
		return nil
	end
	...
	local sound = Instance.new("Sound")
	sound.Name = "PhoenixFlameShield" .. tostring(audioKey)
	sound.SoundId = soundId
	sound.Looped = looped == true
...
function PhoenixClient:PlayPhoenixShieldOneShot(targetPlayer, audioKey)
...
function PhoenixClient:CreatePhoenixRebirthSound(targetPlayer, audioKey)
	local rootPart = getPlayerRootPart(targetPlayer)
	if not rootPart then
		return nil
	end
	...
	local sound = Instance.new("Sound")
	sound.Name = "PhoenixRebirth" .. tostring(audioKey)
	sound.SoundId = soundId
	sound.Looped = false
...
function PhoenixClient:PlayPhoenixRebirthOneShot(targetPlayer, audioKey)
...
function PhoenixClient:PlayPhoenixShieldCueOnce(targetPlayer, cueName, audioKey)
	local state = getOrCreatePhoenixShieldAudioState(self, targetPlayer)
	if not state then
		return nil
	end
...
function PhoenixClient:StartPhoenixShieldLoopSound(targetPlayer)
	local state = getOrCreatePhoenixShieldAudioState(self, targetPlayer)
	if not state then
		return nil
	end
...
function PhoenixClient:StopPhoenixShieldLoopSound(targetPlayer)
	local audioStates = self.phoenixShieldAudioStates
	local state = audioStates and audioStates[targetPlayer] or nil
	stopPhoenixShieldLoopSoundState(state)
end

function PhoenixClient:StopPhoenixShieldAudioForPlayer(targetPlayer, options)
	if not targetPlayer then
		return
	end
...
function PhoenixClient:GetPhoenixFlightAudioMarkers()
	return {
...
function PhoenixClient:BuildPhoenixFlightPresentationPayload(payload)
...
function PhoenixClient:BuildPhoenixRebirthPresentationPayload(payload)
	local presentationPayload = copyPhoenixFlightPayload(payload)
	presentationPayload.AudioReviveSoundOffset = getPhoenixRebirthAudioNumber(self, "ReviveSoundOffset", 0)
	return presentationPayload
end

function PhoenixClient:HandlePhoenixFlightAudioCue(targetPlayer, cueName, cuePayload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function PhoenixClient:HandlePhoenixShieldAudioCue(targetPlayer, cueName, cuePayload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function PhoenixClient:HandlePhoenixRebirthAudioCue(targetPlayer, cueName, _cuePayload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return false
	end
...
function PhoenixClient:ReportPhoenixFlightEnded(reason)
	...
	if typeof(requestAbility) ~= "function" then
		warn(string.format(
			"[PhoenixCooldown] flight end report skipped player=%s userId=%s reason=%s missingRequestAbility=true",
			self.player and self.player.Name or "<nil>",
			tostring(self.player and self.player.UserId),
			resolvedReason
		))
	...
	if not ok then
		warn(string.format(
			"[PhoenixCooldown] flight end report failed player=%s userId=%s reason=%s err=%s",
			self.player and self.player.Name or "<nil>",
			tostring(self.player and self.player.UserId),
			resolvedReason,
			tostring(err)
...
function PhoenixClient:BeginPhoenixFlightTakeoff(rootPart, now)
...
function PhoenixClient:BeginPhoenixFlightControl(rootPart, now)
...
function PhoenixClient:HasReachedPhoenixFlightHeight(rootPart)
	if not rootPart then
		return false
	end
...
function PhoenixClient:TryBeginPhoenixFlightControlAtHeight(rootPart, now)
...
function PhoenixClient:UpdatePhoenixFlightTakeoff(rootPart, humanoid, dt, now)
	if not self.phoenixFlightState.TakeoffStarted then
		return false
	end
...
function PhoenixClient:StartPhoenixFlight(payload)
	local rootPart = getRootPart(self)
	local humanoid = getHumanoid(self)
...
function PhoenixClient:SetFlightInputKeyState(keyCode, isPressed)
...
function PhoenixClient:GetFlightInputAxes()
	local forwardAxis = 0
	local rightAxis = 0
	...
	if not self.flightControls then
		self.flightControls = getPlayerControls(self.player)
	end

	local moveVector = getControlMoveVector(self.flightControls)
	if moveVector.Magnitude <= MIN_FLIGHT_MOVE_INPUT_MAGNITUDE then
		return forwardAxis, rightAxis
	end
...
function PhoenixClient:GetCameraRelativeFlightDirection(rootPart)
...
function PhoenixClient:GetScaledPhoenixFlightSpeed(humanoid)
...
function PhoenixClient:GetTargetHoverVerticalVelocity(currentHeight, now)
...
function PhoenixClient:FaceCharacterTowards(direction, dt)
...
function PhoenixClient:SnapPhoenixHoverHeight(rootPart)
	local character = getCharacter(self)
	if not character or not rootPart then
		return
	end
...
function PhoenixClient:UpdatePhoenixFlight(dt)
	local now = os.clock()
...
function PhoenixClient:BuildHazardOverlapParams()
...
function PhoenixClient:SuppressHazardPart(part, untilTime)
...
function PhoenixClient:SuppressHazardContainer(container, untilTime)
...
function PhoenixClient:RestoreSuppressedHazardParts(now)
...
function PhoenixClient:ClearPhoenixShieldState(targetPlayer, options)
	if not targetPlayer then
		return
	end
...
function PhoenixClient:StopPhoenixRebirthAudioForPlayer(targetPlayer)
	if not targetPlayer then
		return
	end
...
function PhoenixClient:PlayPhoenixShieldGameplayEndAudio(targetPlayer, shieldState)
...
function PhoenixClient:SchedulePhoenixShieldAudioExpiration(targetPlayer, shieldState)
...
function PhoenixClient:HasActivePhoenixShield(now)
...
function PhoenixClient:CreatePhoenixShieldHitPlaceholder(shieldOwner, shield, hazardContainer, hitPart, now)
...
function PhoenixClient:SuppressHazardsNearPhoenixShield(shieldOwner, shield, ownerRootPart, localRootPart, now)
...
function PhoenixClient:UpdatePhoenixShieldHazardSuppression()
	local now = os.clock()
...
function PhoenixClient:EnsurePhoenixShieldHazardSuppressionLoop()
	if self.hazardSuppressionLoopRunning then
		return
	end
...
function PhoenixClient:IsLocalPlayerInsidePhoenixShield(position)
...
function PhoenixClient:ReleasePhoenixShieldAnimationLock(lock)
...
function PhoenixClient:SchedulePhoenixShieldAnimationUnlock(lock)
...
function PhoenixClient:StartPhoenixShieldAnimationLock(payload)
...
function PhoenixClient:StartPhoenixShield(targetPlayer, payload)
	if not targetPlayer or not targetPlayer:IsA("Player") then
		return
	end
...
function PhoenixClient:StopPhoenixVisualsForPlayer(targetPlayer, fadeTime)
...
function PhoenixClient:HandleInputBegan(input, gameProcessed)
...
function PhoenixClient:HandleInputEnded(input)
	return self:SetFlightInputKeyState(input and input.KeyCode, false)
end

function PhoenixClient:BuildRequestPayload(_abilityName, _abilityConfig, fallbackBuilder)
...
function PhoenixClient:BeginPredictedRequest(_abilityName, fallbackBuilder)
	if typeof(fallbackBuilder) == "function" then
		return fallbackBuilder()
	end
...
function PhoenixClient:HandleEffect(targetPlayer, abilityName, payload)
...
function PhoenixClient:HandleStateEvent()
	return false
end

function PhoenixClient:HandleEquipped()
	return false
end

function PhoenixClient:HandleUnequipped()
	self:HandleCharacterRemoving()
	return false
end

function PhoenixClient:Update(dt)
	self:UpdatePhoenixFlight(dt or 0)
end

function PhoenixClient:HandleCharacterRemoving()
...
function PhoenixClient:HandlePlayerRemoving(leavingPlayer)
