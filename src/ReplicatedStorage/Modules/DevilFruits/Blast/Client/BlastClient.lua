...
local BlastClient = {}
BlastClient.__index = BlastClient

local FRUIT_NAME = "Blast Fruit"
...
local DEFAULT_ANIMATION_KEY_BY_ACTION = {
	[BOMU_ACTION_PLANT] = "Blast.Plant",
	[BOMU_ACTION_JUMP] = "Blast.Jump",
}
...
local SOURCE_LABEL = "ReplicatedStorage.Modules.DevilFruits.Bomu.Client.BlastClient"
...
function BlastClient.Create(config, fruitEntry)
	config = config or {}

	local self = setmetatable({}, BlastClient)
...
function BlastClient:BeginPredictedRequest(_abilityName, fallbackBuilder)
...
function BlastClient:BuildRequestPayload(_abilityName, _abilityConfig, fallbackBuilder)
	if typeof(fallbackBuilder) == "function" then
		return fallbackBuilder()
	end
...
function BlastClient:HandleEffect(targetPlayer, abilityName, payload)
...
function BlastClient:HandleCharacterRemoving()
...
function BlastClient:HandlePlayerRemoving(leavingPlayer)
