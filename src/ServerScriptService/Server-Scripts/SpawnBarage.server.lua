...
local CONFIG = {
	...
	LockedPlayerRandomAvoidRadiusMultiplier = 2.75,
	LockedPlayerRandomAvoidMinRadius = 34,
}

local rng = Random.new()
local activeControllers = {}
local targetedPlayerLocksByUserId = {}
...
Players.PlayerRemoving:Connect(function(player)
	targetedPlayerLocksByUserId[player.UserId] = nil
end)
...
local function isPlayerTargetLocked(player)
	return player ~= nil and targetedPlayerLocksByUserId[player.UserId] ~= nil
end

local function reservePlayerTarget(player, source, targetPosition)
	if not player or isPlayerTargetLocked(player) then
		return nil
	end
	...
	targetedPlayerLocksByUserId[player.UserId] = lock
	return lock
end

local function releasePlayerTarget(lock)
	if type(lock) ~= "table" then
		return
	end

	if targetedPlayerLocksByUserId[lock.UserId] == lock then
		targetedPlayerLocksByUserId[lock.UserId] = nil
	end
end

local function isPositionNearLockedPlayerTarget(position)
	if typeof(position) ~= "Vector3" then
		return false
	end

	local radius = math.max(
		tonumber(CONFIG.LockedPlayerRandomAvoidMinRadius) or 0,
		(tonumber(CONFIG.ImpactRadius) or 0) * (tonumber(CONFIG.LockedPlayerRandomAvoidRadiusMultiplier) or 1)
	)
	for _, lock in pairs(targetedPlayerLocksByUserId) do
		local player = lock and lock.Player
		local _, _, rootPart = player and getCharacterContext(player)
		if rootPart and (rootPart.Position - position).Magnitude <= radius then
			return true
		end

		local targetPosition = lock and lock.TargetPosition
		if typeof(targetPosition) == "Vector3" and (targetPosition - position).Magnitude <= radius then
			return true
		end
	end
...
local function getZoneProfileIndex(zoneIndex)
	...
	if CONFIG.ReverseZoneDifficulty == true then
		if normalizedZoneIndex == 1 then
			return 1
		end

		return math.clamp(2 + (zoneCount - normalizedZoneIndex), 1, zoneCount)
	end
...
local function choosePlayerTargetPosition(refs, startPart, endPart, leftBound, rightBound, zoneIndex)
	...
	for _, player in ipairs(Players:GetPlayers()) do
		...
		if humanoid and rootPart and not isPlayerTargetLocked(player) and not isSukeHardToTarget(player) then
	...
	local source = string.format("player:%s:%.2f", chosen.Player.Name, chosen.ForwardAlpha)
	local targetLock = reservePlayerTarget(chosen.Player, source, chosen.Position)
	if not targetLock then
		return nil, normalizedZoneIndex, "player_target_already_locked"
	end
...
local function chooseZonePosition(refs, startPart, endPart, leftBound, rightBound, zoneIndex)
	...
	for _ = 1, attempts do
		...
		if groundPosition and not isPositionNearLockedPlayerTarget(groundPosition) then
...
local function chooseImpactPosition(refs, startPart, endPart, leftBound, rightBound, zoneIndex)
	local targetPosition, targetZoneIndex, targetSource, targetLock =
		choosePlayerTargetPosition(refs, startPart, endPart, leftBound, rightBound, zoneIndex)
	if targetPosition then
		return targetPosition, targetZoneIndex, targetSource, targetLock
	end
...
local function unregisterController(controller)
	...
	if controller.TargetLock then
		releasePlayerTarget(controller.TargetLock)
		controller.TargetLock = nil
	end
...
local function spawnBarrageCycle()
	...
	for zoneIndex = minimumZoneIndex, zoneCount do
		...
		while shotIndex < spawnsThisZone and spawnAttempts < maxSpawnAttempts do
			...
			spawnAttempts += 1
			local impactPosition, normalizedZoneIndex, source, targetLock = chooseImpactPosition(
				refs,
				startPart,
				endPart,
				leftBound,
			...
			task.delay(launchDelay, function()
				...
				if not hazardsFolder.Parent then
					releasePlayerTarget(targetLock)
					return
				end
				...
				controller.TargetLock = targetLock
