...
local function stripSchemaVersion(root)
	local copy = if typeof(root) == "table" then deepCopy(root) else {}
	copy.SchemaVersion = nil
	return copy
end

local CANONICAL_FIRST_AUDIT_ROOT = "CrewMemberSaveLoadAuthorityAudit"

local function buildCanonicalFirstSnapshot(root)
	return {
		BrainrotInventory = deepCopy(root[CrewProfileSchema.LegacyKeys.Inventory]),
		BrainrotQuickSlots = deepCopy(root[CrewProfileSchema.LegacyKeys.QuickSlots]),
		BrainrotStorage = deepCopy(root[CrewProfileSchema.LegacyKeys.QuickSlotsLegacy]),
		IncomeBrainrots = deepCopy(root[CrewProfileSchema.LegacyKeys.Income]),
		Inventory = deepCopy(root.Inventory),
		StandsLevels = deepCopy(root.StandsLevels),
		Audit = deepCopy(root[CANONICAL_FIRST_AUDIT_ROOT]),
	}
end

local function restoreCanonicalFirstSnapshot(root, snapshot)
	if typeof(root) ~= "table" or typeof(snapshot) ~= "table" then
		return false
	end
	root[CrewProfileSchema.LegacyKeys.Inventory] = deepCopy(snapshot.BrainrotInventory)
	root[CrewProfileSchema.LegacyKeys.QuickSlots] = deepCopy(snapshot.BrainrotQuickSlots)
	root[CrewProfileSchema.LegacyKeys.QuickSlotsLegacy] = deepCopy(snapshot.BrainrotStorage)
	root[CrewProfileSchema.LegacyKeys.Income] = deepCopy(snapshot.IncomeBrainrots)
	root.Inventory = deepCopy(snapshot.Inventory)
	root.StandsLevels = deepCopy(snapshot.StandsLevels)
	root[CANONICAL_FIRST_AUDIT_ROOT] = deepCopy(snapshot.Audit)
	return true
end

local function updateCanonicalFirstAudit(root, updates)
	if typeof(root) ~= "table" then
		return nil
	end
	local audit = if typeof(root[CANONICAL_FIRST_AUDIT_ROOT]) == "table"
		then deepCopy(root[CANONICAL_FIRST_AUDIT_ROOT])
		else {}
	for key, value in pairs(if typeof(updates) == "table" then updates else {}) do
		audit[key] = value
	end
	audit.UpdatedAt = os.time()
	root[CANONICAL_FIRST_AUDIT_ROOT] = audit
	return audit
end

local function validateCanonicalFirstRoots(root)
	local status = getCanonicalKeyStatus(root)
	if status.CrewMemberInventoryShape ~= true then
		return false, "canonical_inventory_invalid", status
	end
	if status.CrewMemberQuickSlotsShape ~= true then
		return false, "canonical_quick_slots_invalid", status
	end
	if status.CrewMemberIncomeShape ~= true then
		return false, "canonical_income_invalid", status
	end
	return true, nil, status
end

local function rebuildLegacyMirrorsFromCanonical(root)
	local canonicalQuickSlots = root[CrewProfileSchema.Keys.QuickSlots]

	root[CrewProfileSchema.LegacyKeys.QuickSlots] = stripSchemaVersion(canonicalQuickSlots)
...
function CrewMemberShadowWriter.ProjectCanonicalToLegacyMirrors(source, options)
	local root = getRoot(source)
	local flags = getFlags(options)
	local actor = getActorName(options)
	options = if typeof(options) == "table" then options else {}
	local reason = tostring(options.Reason or "canonical_first_save_load")
	...
	if flags.CrewMemberSaveLoadCanonicalFirstEnabled ~= true then
		return {
			Actor = actor,
			Reason = reason,
			Mode = "legacy_fallback",
			DidWrite = false,
			Flags = flags,
			SkippedReason = "canonical_first_disabled",
		}
	end

	local canonicalOk, fallbackReason, canonicalStatus = validateCanonicalFirstRoots(root)
	if canonicalOk ~= true then
		updateCanonicalFirstAudit(root, {
			Mode = "legacy_fallback",
	...
	local snapshot = buildCanonicalFirstSnapshot(root)
	local rootsRebuilt = rebuildLegacyMirrorsFromCanonical(root)
	...
	if validation.BlockingIssueCount ~= 0 then
		local restored = restoreCanonicalFirstSnapshot(root, snapshot)
		updateCanonicalFirstAudit(root, {
			Mode = "legacy_fallback",
			FallbackReason = "canonical_first_validation_failed",
			RootsRebuilt = rootsRebuilt,
			Validation = validationStatus,
			RestoredSnapshot = restored == true,
			BlockingCount = validation.BlockingIssueCount,
			UnclassifiedCount = 0,
		})
		return CrewMemberShadowWriter.ProjectLegacyToCanonicalShadow(source, options)
	end

	local audit = updateCanonicalFirstAudit(root, {
		Mode = "canonical_first",
		FallbackReason = nil,
		RootsRebuilt = rootsRebuilt,
		Validation = validationStatus,
		MirrorRepairCount = #rootsRebuilt,
		BlockingCount = validation.BlockingIssueCount,
		UnclassifiedCount = 0,
	})
	...
	if flags.CrewMemberShadowReportEnabled == true then
		print(string.format(
			"%s reason=%s actor=%s mode=canonical_first rebuilt=%d blocking=%d unclassified=%d canonicalRead=%s gameplayReads=%s profileMigrationWrite=%s",
			LOG_PREFIX,
			reason,
			actor,
			#rootsRebuilt,
...
function CrewMemberShadowWriter.RunProfileReadyProjection(source, options)
	...
	if flags.CrewMemberSaveLoadCanonicalFirstEnabled == true then
		...
		options.Flags = flags
		options.Reason = tostring(options.Reason or "profile_ready_canonical_first")
		return CrewMemberShadowWriter.ProjectCanonicalToLegacyMirrors(source, options)
	end
	return CrewMemberShadowWriter.ProjectLegacyToCanonicalShadow(source, options)
end
