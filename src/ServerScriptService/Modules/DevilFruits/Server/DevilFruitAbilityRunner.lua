...
function DevilFruitAbilityRunner.Execute(params)
	...
	params.Security.LogExecutionStage(
		...
		abilityName,
		"activated",
		string.format("nextReadyAt=%s applyCooldown=%s", tostring(nextReadyAt), tostring(applyCooldown)),
		payload
	)
	if shouldLogMeraAudit(fruitName, abilityName) then
		logMeraAudit(
			"INFO",
			"Mera ability runner activated player=%s fruit=%s ability=%s nextReadyAt=%s applyCooldown=%s",
			player and player.Name or "<nil>",
			tostring(fruitName),
			tostring(abilityName),
