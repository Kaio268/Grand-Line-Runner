	...
	task.delay(BODY_DEATH_RESPAWN_DELAY, function()
		...
		then
			...
			if queued == false then
				warn(("[SpiritServer] Failed to queue ship respawn for %s after body death: %s"):format(
					player.Name,
					tostring(reason)
				))
			end
				...
				if probeBodyHazards(state) then
					if not isDamageProtected(state.Player, ghostRoot.Position, "HoroBodyHazard") then
						DamageProtection.TraceMoguStartupDamage(state.Player, {
							...
							Path = "SpiritServer.monitorProjection.bodyHazard",
						})
...
local function handleActionRemote(player, actionName, payload)
	...
	if actionName == ACTION_BODY_HAZARD then
		if isDamageProtected(state.Player, state.GhostRoot and state.GhostRoot.Position or nil, "SpiritClientBodyHazard") then
			return
		end
		...
		if state.Humanoid and state.Humanoid.Parent and state.Humanoid.Health > 0 then
			DamageProtection.TraceMoguStartupDamage(state.Player, {
				...
				Source = "SpiritClientBodyHazard",
				Path = "SpiritServer.handleActionRemote.bodyHazard",
			})
...
function SpiritServer.ShouldBypassCooldownCheck(selfOrContext, maybeContext)
...
function SpiritServer.ShouldBypassRequestThrottle(selfOrContext, maybeContext)
	local context = resolveAbilityContext(selfOrContext, maybeContext)
	local activeState = getManualCancelProjectionState(context)
...
function SpiritServer.GhostProjection(context)
...
function SpiritServer.IsProjecting(player)
...
function SpiritServer.ClearRuntimeState(player)
	local state = activeProjectionsByPlayer[player]
	if state then
...
function SpiritServer.GetLegacyHandler()
	return SpiritServer
end
