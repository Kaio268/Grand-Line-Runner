...
local InfernoShared = Modules:WaitForChild("DevilFruits"):WaitForChild("Inferno"):WaitForChild("Shared")
local MeraDashShared = require(InfernoShared:WaitForChild("MeraDashShared"))
...
local function logMove(player, message, ...)
	...
	if not DiagnosticLogLimiter.ShouldEmit("InfernoServer:MOVE", DiagnosticLogLimiter.BuildKey(message, ...), MOVE_LOG_COOLDOWN) then
		return
	end
...
local function logFireBurst(player, message, ...)
	...
	if not DiagnosticLogLimiter.ShouldEmit("InfernoServer:FIREBURST", DiagnosticLogLimiter.BuildKey(message, ...), MOVE_LOG_COOLDOWN) then
		return
	end
...
function InfernoServer.FlameDash(context)
...
function InfernoServer.FireBurst(context)
...
function InfernoServer.ClearRuntimeState(player)
	if player and player.Parent then
