...
local MeraAnimationResolver = require(
	ReplicatedStorage:WaitForChild("Modules"):WaitForChild("DevilFruits"):WaitForChild("Inferno"):WaitForChild("Shared"):WaitForChild("MeraAnimationResolver")
)
...
local function playAnimation(character, moveName, animationConfig, defaultAssetName, defaultAnimationKey)
	...
	if not animator then
		...
		logAnimPipeline("WARN", "server animator missing fruit=%s move=%s path=%s", "Inferno Fruit", tostring(moveName), tostring(animationPath))
		return nil
	end
	logAnimPipeline("INFO", "server animator ready fruit=%s move=%s asset=%s", "Inferno Fruit", tostring(moveName), tostring(animationKey or legacyAssetName))
	...
	if not selectedTrack then
		...
		logAnimPipeline("WARN", "server animation load failed fruit=%s move=%s path=%s detail=%s", "Inferno Fruit", tostring(moveName), tostring(animationPath), tostring(detail))
	...
	logAnimPipeline("INFO", "server animation play reached fruit=%s move=%s path=%s", "Inferno Fruit", tostring(moveName), tostring(selectedCandidate and selectedCandidate.Path))
	logInfo(
...
function MeraAnimationController.PlayFlameDashAnimation(character, animationConfig)
	return playAnimation(character, "FlameDash", animationConfig, "Flame Dash", "Inferno.FlameDash")
end

function MeraAnimationController.PlayFireBurstAnimation(character, animationConfig)
	return playAnimation(character, "FireBurst", animationConfig, "Flame burst", "Inferno.FlameBurstR6")
end
