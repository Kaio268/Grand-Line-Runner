...
local cachedPhoenixPassiveService = nil
...
local function getPhoenixPassiveService()
	if cachedPhoenixPassiveService then
		return cachedPhoenixPassiveService
	end

	cachedPhoenixPassiveService = require(script.Parent:WaitForChild("PhoenixPassiveService"))
	return cachedPhoenixPassiveService
end

function PhoenixServer.PhoenixFlight(context)
...
function PhoenixServer.PhoenixFlameShield(context)
	local abilityConfig = context.AbilityConfig
...
function PhoenixServer.ShouldBypassCooldownCheck(selfOrContext, maybeContext)
...
function PhoenixServer.ShouldBypassRequestThrottle(selfOrContext, maybeContext)
	return PhoenixServer.ShouldBypassCooldownCheck(selfOrContext, maybeContext)
end

function PhoenixServer.IsPhoenixFlightActive(player)
...
function PhoenixServer.GetProtection(player, position)
	if typeof(player) ~= "Instance" or not player:IsA("Player") then
		return nil
	end
	...
	local passiveService = getPhoenixPassiveService()
...
function PhoenixServer.IsProtected(player, position)
	local protection = PhoenixServer.GetProtection(player, position)
	if protection then
...
function PhoenixServer.ClearRuntimeState(player, _fruitName, cleanupReason)
...
function PhoenixServer.GetLegacyHandler()
