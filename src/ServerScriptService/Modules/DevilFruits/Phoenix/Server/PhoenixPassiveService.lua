...
local function getPhoenixRebirthConfig()
	local fruitConfig = DevilFruitConfig.GetFruit(PhoenixShared.FruitName)
...
local function isPhoenixEquipped(player)
	return DevilFruitService.GetEquippedFruit(player) == PhoenixShared.FruitName
end
...
local function triggerPhoenixRebirth(player, state, reason)
	...
	if state.Used or state.Reviving or not isPhoenixEquipped(player) then
		return false
	end
...
function PhoenixPassiveService.Start()
	if started then
		return
	end
...
function PhoenixPassiveService.ClearRuntimeState(player)
...
function PhoenixPassiveService.IsProtected(player)
	local state = statesByPlayer[player]
...
function PhoenixPassiveService.TryConsumeRebirth(player, reason)
	local state = statesByPlayer[player]
	if not state and player and player.Character then
		bindCharacter(player, player.Character)
		state = statesByPlayer[player]
