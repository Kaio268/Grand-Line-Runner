...
local function playAnimation(character, animationConfig, defaultAnimationKey, moveName)
	...
	if not animator then
		logAnimPipeline("WARN", "server animator missing fruit=%s move=%s asset=%s", "Glacial Fruit", tostring(resolvedMoveName), tostring(animationKey))
		return nil
	end
	logAnimPipeline("INFO", "server animator ready fruit=%s move=%s asset=%s", "Glacial Fruit", tostring(resolvedMoveName), tostring(animationKey))
	...
	if not track then
		...
		logAnimPipeline("WARN", "server animation load failed fruit=%s move=%s asset=%s detail=%s", "Glacial Fruit", tostring(resolvedMoveName), tostring(animationKey), tostring(loadFailure))
		return nil
	end
	...
	logAnimPipeline("INFO", "server animation track created fruit=%s move=%s asset=%s", "Glacial Fruit", tostring(resolvedMoveName), tostring(animationKey))
	...
	logAnimPipeline("INFO", "server animation play reached fruit=%s move=%s asset=%s", "Glacial Fruit", tostring(resolvedMoveName), tostring(animationKey))
...
function HieAnimationController.PlayFreezeShotAnimation(character, animationConfig)
	return playAnimation(character, animationConfig, "Glacial.IceBlast", "IceBlast")
end
...
function HieAnimationController.PlayIceBoostAnimation(player, character, animationConfig, token)
	...
	local animationState = playAnimation(character, animationConfig, "Glacial.IceBoost", "IceBoost")
...
function HieAnimationController.StopIceBoostAnimation(player, token, _reason)
