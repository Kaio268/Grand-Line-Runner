...
local function logWarn(message, ...)
	if not DiagnosticLogLimiter.ShouldEmit("BurrowServer:WARN", DiagnosticLogLimiter.BuildKey(message, ...), WARN_COOLDOWN) then
		return
	end
...
local function logInfo(message, ...)
	...
	if not DiagnosticLogLimiter.ShouldEmit("BurrowServer:INFO", DiagnosticLogLimiter.BuildKey(message, ...), INFO_COOLDOWN) then
		return
	end
...
local function setStartupInvincibilityState(player, burrowState)
	...
	DamageProtection.TraceMoguStartupDamage(player, {
		Path = "BurrowServer.setStartupInvincibilityState",
		Source = "BurrowServer",
	})
end
	...
	if humanoid then
		connections[#connections + 1] = humanoid.Died:Connect(function()
			DamageProtection.TraceMoguStartupDamage(player, {
				...
				Path = "BurrowServer.HumanoidDied",
...
function BurrowServer.Burrow(context)
	...
	DamageProtection.TraceMoguStartupDamage(player, {
		...
		Path = "BurrowServer.BurrowAccepted",
		Source = "BurrowServer",
	})
...
function BurrowServer.IsPlayerUnderground(player)
	if not player or not player:IsA("Player") then
		return false
	end
...
function BurrowServer.IsMoguBurrowed(player)
	return BurrowServer.IsPlayerUnderground(player)
end

function BurrowServer.IsStartupInvincible(player)
	local activeBurrow = getActiveBurrow(player)
...
function BurrowServer.GetProtection(player, position, _options)
	if not player or not player:IsA("Player") then
		return nil
	end
...
function BurrowServer.IsProtected(player)
	local protection = BurrowServer.GetProtection(player)
	if protection then
...
function BurrowServer.EnforceProtectionIntegrity(player, reason)
