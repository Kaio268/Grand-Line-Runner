...
local function unlock(player, titleId)
	...
	if not ok then
		warn(string.format("[TitleProgressService] Unlock failed player=%s title=%s reason=%s", player.Name, tostring(titleId), tostring(result)))
		return false
	end
...
function TitleProgressService.RecordFruitEaten(player, fruitName)
	...
	elseif isFruitKey(fruitName, "Tori") then
