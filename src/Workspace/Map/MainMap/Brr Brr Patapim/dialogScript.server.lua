...
local function getTutorialValue()
	...
	local tv = hls:WaitForChild("Tutorial")
	return tv
end
...
ProximityPromptService.PromptTriggered:Connect(function(p, triggeredPlayer)
	if triggeredPlayer and triggeredPlayer ~= player then
		return
	end
	table.insert(promptQueue, p)
end)
...
local function disconnectPoint()
	...
	if xConn then xConn:Disconnect() xConn = nil end
end
...
local function ensureBeam(character)
	...
	beam = ReplicatedStorage.Beam:Clone()
...
local function getNearestModel(character)
	...
	local function considerModel(m)
		...
		seen[m] = true
		if not modelHasPrompt(m) then return end
	...
	for _, obj in ipairs(container:GetChildren()) do
		...
		for _, desc in ipairs(obj:GetDescendants()) do
			if desc:IsA("Model") then considerModel(desc) end
		end
	end
...
local function getBrrMesh()
	local map = workspace:WaitForChild("Map")
	local mainMap = map:WaitForChild("MainMap")
	local container = mainMap:WaitForChild("Brr Brr Patapim")
	return container:WaitForChild("Mesh")
end
...
local function getStand1Handle()
	...
	if not stands then return nil, nil end

	local stand1 = stands:FindFirstChild("1")
	if not stand1 then return nil, nil end
...
local beamStage = "OFF" -- "TO_BRR" | "BRAINROT" | "WAIT_FOR_FOLDER" | "PLOT_STAND"
...
local function startInitialBeamToBrr()
	if tutorialDisabled then return end
	if finishedSent then return end
		...
		elseif beamStage == "BRAINROT" then
			...
			if not model then
				...
				while beamActive and beamStage == "BRAINROT" and character.Parent and isAlive(character) do
					...
					local p = popPrompt()
					if p then
						if (currentBrainrotPrompt and p == currentBrainrotPrompt)
							or (currentBrainrotModel and p:IsDescendantOf(currentBrainrotModel)) then
		...
		elseif beamStage == "PLOT_STAND" then
			...
			if not handle then
				...
				while beamActive and beamStage == "PLOT_STAND" and character.Parent and isAlive(character) do
					...
					local p = popPrompt()
					if p then
						if (handlePrompt and p == handlePrompt) or p:IsDescendantOf(handle) then
							completeTutorial()
							return
						end
					else
...
local function startSpeedUpgradeTutorial()
	...
	task.spawn(function()
		...
		buyConn = buyButton.Activated:Connect(function()
			if tutorialDisabled or finishedSent then return end
...
npcPrompt.Triggered:Connect(function(plr)
	dialogObject:triggerDialog(plr, 1)
end)
